...
/* ─────────────────────────────────────────────
   ...
   The real Product type (app/lib/data.ts) has no
   accentColor field, so we derive one from the
   collection name instead.
───────────────────────────────────────────── */
type Accent = "gold" | "rose" | "olive";

const ACCENT_BY_COLLECTION: Record<string, Accent> = {
  "Nocturne Reserve": "gold",
  "Daylight Atelier": "olive",
  "Bloom Circuit": "rose",
};

function accentFor(collection: string): Accent {
  return ACCENT_BY_COLLECTION[collection] ?? "gold";
}
...
const COLLECTION_META: Record<
  string,
  { tagline: string; desc: string; mood: string }
> = {
  All: {
    tagline: "Every drop, every mood.",
    desc: "Curated fragrances across three collections — each chosen for lasting impressions.",
    mood: "All collections",
  },
  "Nocturne Reserve": {
    tagline: "For those who arrive after dark.",
    desc: "Deep oud, smoked amber, and dark woods. Fragrances that command a room.",
    mood: "Dark · Intense · Memorable",
  },
  "Daylight Atelier": {
    tagline: "Light enough to wear all day.",
    desc: "Fresh citrus, clean musks, and green notes. Made for the hours between sunrise and sunset.",
    mood: "Fresh · Clean · Effortless",
  },
  "Bloom Circuit": {
    tagline: "Soft. Feminine. Unforgettable.",
    desc: "Roses, tuberose, jasmine, and powder. Every bottle built around a floral heart.",
    mood: "Floral · Romantic · Soft",
  },
};

function notesToLine(notes: string[]): string {
  return notes.join(" · ");
}
...
function CollectionsInner() {
  ...
  return (
    ...
    Left: text + filters
    Right: video (replaces the bottle SVG)
════════════════════════════════ */}
        <section
          ...
          style={{
            minHeight: "60vh",
            background:
              "radial-gradient(ellipse 80% 60% at 30% 100%,#1E160A 0%,var(--le-obsidian) 65%)",
          }}
        >
          ...
          {/* small accent bottle — bottom left, desktop only */}
          <div
            className="absolute left-[3%] bottom-[18%] le-float hidden xl:block"
          ...
          <div className="relative z-10 grid grid-cols-1 lg:grid-cols-2 min-h-[60vh]">
            {/* LEFT — copy + filters */}
            <div className="flex flex-col justify-end px-8 md:px-20 pb-16 pt-28 lg:pt-20">
              <p
                className={`flex items-center gap-4 text-[.68rem] tracking-[.32em] uppercase mb-6 transition-all duration-700 ${visible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-6"}`}
                style={{ color: "var(--le-gold)", transitionDelay: ".1s" }}
              >
                <span
                  className="inline-block w-8 h-[1px]"
                  style={{ background: "var(--le-gold)" }}
                />
              ...
              >
                ...
                <br />
                tells a{" "}
                <em style={{ color: "var(--le-gold-light)" }}>story.</em>
              </h1>
              <p
                className={`text-[.95rem] leading-[1.85] max-w-[32rem] mb-10 transition-all duration-700 ${visible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-6"}`}
                style={{ color: "var(--le-mist)", transitionDelay: ".32s" }}
              >
                Curated fragrances across three moods. Choose your scent,
                message us on WhatsApp, and we'll deliver to your door.
              </p>
              <div
                    ...
                    style={{
                      background:
                        active === c
                          ...
                          : "rgba(201,169,110,.08)",
                      color:
                        active === c ? "var(--le-obsidian)" : "var(--le-mist)",
                      border:
                        active === c
                          ? "1px solid var(--le-gold)"
            ...
            {/* RIGHT — video panel (replaces HeroBannerBottle SVG) */}
            <div
              className="relative hidden lg:block overflow-hidden"
              aria-hidden="true"
            >
              {/* ── VIDEO ─────────────────────────────────────────────
          Source: /public/videos/video-0.mp4
          poster: /public/videos/video-0-poster.jpg
          Sits on the right half, full height, object-cover.
          Left-edge gradient blends it into the dark copy side.
      ──────────────────────────────────────────────────────── */}
              <video
                className="absolute inset-0 w-full h-full object-cover"
                src="/videos/video-1.mp4"
                poster="/videos/video-0-poster.jpg"
                autoPlay
                muted
                loop
                playsInline
                preload="none"
              />
              {/* left-edge blend into obsidian */}
              <div
                className="absolute inset-0 pointer-events-none"
                style={{
                  background:
                    "linear-gradient(to right,var(--le-obsidian) 0%,transparent 30%,transparent 80%,rgba(10,8,6,.3) 100%)",
                }}
              />
        ...
        ════════════════════════════════ */}
        <section
          className="px-8 md:px-20 py-14"
          style={{ background: "var(--le-obs2)" }}
        >
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
            <div>
              <p
                className="le-reveal text-[.68rem] tracking-[.28em] uppercase mb-3"
                style={{ color: "var(--le-gold)" }}
              >
                {meta.mood}
              </p>
              <h2
                className="le-display le-reveal le-d1 font-light leading-[1.05]"
                style={{
              ...
              <p
                className="le-reveal le-d2 mt-3 text-[.9rem] leading-[1.8] max-w-[36rem]"
            ...
            <div className="le-reveal le-d2 shrink-0 text-right">
              <span
                className="le-display text-[3rem] font-light leading-none"
                style={{ color: "var(--le-gold-light)" }}
              >
                {filtered.length}
              </span>
              <span
                className="block text-[.68rem] tracking-[.16em] uppercase mt-1"
                style={{ color: "var(--le-mist)" }}
              >
        ...
        >
          {filtered.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-32 text-center">
              <p
                className="le-display text-[2rem] font-light mb-4"
                style={{ color: "var(--le-mist)" }}
              >
                No fragrances found
              </p>
              <button
                ...
                className="text-[.72rem] tracking-[.14em] uppercase px-6 py-3 rounded-[1px] mt-2"
                style={{
        ...
        >
          <p
            className="le-reveal flex items-center gap-4 text-[.68rem] tracking-[.28em] uppercase mb-10"
            style={{ color: "var(--le-gold)" }}
          >
            <span
              className="inline-block w-8 h-[1px]"
            ...
            {COLLECTIONS.filter((c) => c !== "All").map((c, i) => {
              ...
              return (
                <button
                  ...
                  style={{
                    background:
                      active === c ? "var(--le-obs3)" : "var(--le-obs2)",
                  }}
                  onMouseEnter={(e) =>
                ...
                >
                  <span
                    className="block text-[.68rem] tracking-[.22em] uppercase mb-3"
                    style={{ color: colors[i] }}
                  >
                    0{i + 1}
                  </span>
                  <span
                    className="le-display block font-light leading-[1.05] mb-2"
                    style={{
                  ...
                  <span
                    className="block text-[.78rem]"
                    style={{ color: "var(--le-mist)" }}
                  >
                    {COLLECTION_META[c].mood}
                  </span>
                  <span
                    className="block mt-6 text-[.68rem] tracking-[.16em] uppercase transition-all duration-300 group-hover:translate-x-1"
                    style={{ color: colors[i] }}
                  >
                    Browse →
                  </span>
        ...
        ════════════════════════════════ */}
        <section
          className="relative overflow-hidden px-8 md:px-20 py-24 text-center"
          style={{
            background:
        ...
        >
          ...
          <div className="relative z-10">
            <p
              className="le-reveal flex items-center justify-center gap-4 text-[.68rem] tracking-[.28em] uppercase mb-6"
              style={{ color: "var(--le-gold)" }}
            >
              <span
                className="inline-block w-8 h-[1px]"
            ...
            <h2
              className="le-display le-reveal le-d1 font-light leading-[1.05] mb-6"
              style={{
            ...
            >
              ...
              <br />
              <em style={{ color: "var(--le-gold-light)" }}>
                We'll help you decide.
              </em>
            </h2>
            <p
              className="le-reveal le-d2 text-[.92rem] leading-[1.9] max-w-[30rem] mx-auto mb-10"
              style={{ color: "var(--le-mist)" }}
            >
              Tell us the occasion, your budget, and the notes you love. We'll
              send a personalised recommendation straight to your WhatsApp.
            </p>
            <a
              href={`https://wa.me/${CONTACT.ordersWhatsApp}?text=Hello%20L'Essence%2C%20I%20need%20a%20scent%20recommendation.`}
              ...
              onMouseEnter={(e) => {
                e.currentTarget.style.background = "var(--le-gold-light)";
      ...
      {modal && (
        ...
        >
          <div
            className="le-modal-in relative w-full max-w-[860px] max-h-[90vh] overflow-auto rounded-[2px]"
            style={{
          ...
          >
            <button
              ...
              onMouseEnter={(e) =>
                (e.currentTarget.style.color = "var(--le-gold)")
              }
              onMouseLeave={(e) =>
                (e.currentTarget.style.color = "var(--le-mist)")
              }
            >
              ✕ Close
            </button>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2">
              ...
              >
                {modal.image && !imgErrors[modal.id] ? (
                  <Image
                    ...
                    onError={() =>
                      setImgErrors((prev) => ({ ...prev, [modal.id]: true }))
                    }
                  />
                ) : (
                  <div className="absolute inset-0 flex items-center justify-center">
                    <LargeBottleSvg color={accentFor(modal.collection)} />
                  </div>
                )}
                <div
                  className="absolute inset-0 pointer-events-none"
                  style={{
                    background:
                      "linear-gradient(to right,transparent 60%,var(--le-obs2))",
                  }}
                />
              </div>
              ...
              <div className="flex flex-col justify-center gap-6 p-8 md:p-12">
                <div>
                  <p
                    className="text-[.65rem] tracking-[.22em] uppercase mb-3"
                    style={{ color: "var(--le-gold)" }}
                  >
                    {modal.collection}
                  </p>
                  <h2
                    className="le-display font-light leading-[1.05]"
                    style={{
                ...
                {modal.notes.length > 0 && (
                  <div className="flex flex-wrap gap-2">
                    {modal.notes.map((note) => (
                      <span
                        key={note}
                        className="text-[.7rem] tracking-[.08em] px-3 py-1.5 rounded-[1px]"
                        style={{
                ...
                <p
                  className="le-display text-[2.2rem] font-light"
                  style={{ color: "var(--le-gold-light)" }}
                >
                ...
                <div className="flex flex-col gap-3">
                  <a
                    ...
                    rel="noopener noreferrer"
                  >
                    <WhatsAppIcon size={18} color="var(--le-obsidian)" /> Order
                    on WhatsApp
                  </a>
                  <button
                    onClick={() => setModal(null)}
                ...
                <p
                  className="text-[.72rem] leading-[1.7]"
                  style={{ color: "var(--le-mist)" }}
                >
                  Message us to confirm availability and arrange same-day or
...
function ProductCard({
  ...
  return (
    <article
      className={`card-glow le-reveal ${delayClass} group relative flex flex-col transition-colors duration-300`}
      style={{ background: "var(--le-obs2)" }}
      onMouseEnter={(e) =>
        ((e.currentTarget as HTMLElement).style.background = "var(--le-obs3)")
      }
      onMouseLeave={(e) =>
        ((e.currentTarget as HTMLElement).style.background = "var(--le-obs2)")
      }
    >
      <div className="relative overflow-hidden" style={{ aspectRatio: "3/4" }}>
        {product.image && !imgError ? (
          <>
            ...
            <div
              ...
              style={{
                background:
                  "linear-gradient(to bottom,rgba(10,8,6,.18) 0%,rgba(10,8,6,.72) 100%)",
              }}
            />
          </>
        ) : (
          <div
            ...
            style={{
              background: `radial-gradient(ellipse at 50% 60%,rgba(${accentRgb},.12) 0%,var(--le-obs3) 70%)`,
            }}
          >
            <LargeBottleSvg color={accent} />
          </div>
        )}

        {product.bestseller && (
          <span
            className="absolute top-4 left-4 text-[.6rem] tracking-[.18em] uppercase px-3 py-1.5 rounded-[1px]"
            style={{
        ...
        >
          <span
            className="text-[.7rem] tracking-[.18em] uppercase px-5 py-3 rounded-[1px]"
            style={{
      ...
      <div className="flex flex-col flex-1 p-6">
        <p
          className="text-[.62rem] tracking-[.18em] uppercase mb-2"
          style={{ color: accentVar }}
        >
          {product.collection}
        </p>
        <h3
          className="le-display font-light text-[1.45rem] leading-[1.1] mb-2"
          style={{ color: "var(--le-white)" }}
        >
          {product.name}
        </h3>
        {notesLine && (
          <p
            className="text-[.75rem] leading-[1.7] mb-4"
            style={{ color: "var(--le-mist)" }}
          >
            {notesLine}
          </p>
        ...
        >
          <span
            className="le-display text-[1.4rem] font-light"
            style={{ color: "var(--le-gold-light)" }}
          >
  ...
  return (
    <svg width="90" height="200" viewBox="0 0 90 200" fill="none" opacity=".35">
      ...
      <rect
        ...
        height="140"
        rx="3"
        fill="rgba(201,169,110,.12)"
        stroke="rgba(201,169,110,.3)"
        strokeWidth=".5"
      />
      <rect
        x="24"
...
function LargeBottleSvg({ color }: { color?: Accent }) {
  const rgb =
    color === "rose"
      ? "196,150,140"
      : color === "olive"
    ...
    <svg width="100" height="186" viewBox="0 0 100 186" fill="none">
      <defs>
        <linearGradient id={`lg-${color}`} x1="0" y1="0" x2="0" y2="1">
          <stop offset="0%" stopColor={`rgba(${rgb},.5)`} />
          <stop offset="100%" stopColor={`rgba(${rgb},.15)`} />
        </linearGradient>
      </defs>
      <rect
        x="37"
...
function PageSkeleton() {
  return (
    ...
    >
      <div
        className="le-display text-[1.2rem] tracking-[.2em] font-light"
        style={{ color: "var(--le-mist)" }}
      >
        Loading collections…
      </div>
